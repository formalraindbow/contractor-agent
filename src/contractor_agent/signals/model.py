...
class Verdict(StrEnum):
    """Три исхода — по кейсодателю (QA 2.09, 43:14), в форме условий, а не ярлыка.

    Гайды по текстам и `docs/CRITERIA.md` §2: вывод — действие для пользователя
    («предоплата и документы»), не характеристика контрагента («не рекомендуем работать»).
    Коды в контракте с API и фронтом не меняются.
    """

    OK = "ok"  # можно работать
    CHECK = "check"  # до договора проверить
    NOT_RECOMMENDED = "not_recommended"  # работать только на условиях
