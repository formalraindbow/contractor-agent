...
- verdict_instead_of_conditions (−1): категоричная оценка компании — «работать нельзя», «не рекомендуем», «надёжная/ненадёжная компания», «есть существенные риски». Штатные формулировки снижением НЕ считаются: «можно работать» для исхода ok, «нужна дополнительная проверка», «в отчёте есть факты, требующие особого внимания»;
...
- comparison (несколько компаний): у каждой компании должен быть свой вывод буквально по её verdict_ru и один-два факта; пропущенный красный факт (банкротство, исключение из реестра, блокировка счетов) хотя бы у одной компании — missed_critical; рейтинг, баллы, места вместо решений «с кем работать / кого проверить» — verdict_instead_of_conditions;
