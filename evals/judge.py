...
- courts_without_role_period_field (−2): названо число судебных дел БЕЗ роли (истец/ответчик) ИЛИ БЕЗ статуса или периода («открытые», «завершённые», год — это статус/период) ИЛИ без поля-источника (в тексте или в списке цитат); нужны роль, статус/период и источник;
...
- verdict_instead_of_conditions (−1): категоричный приговор вместо условия — «работать нельзя», «не рекомендуем», «надёжная/ненадёжная компания». Три штатные формулировки исхода снижением НЕ считаются: «можно работать», «стоит проверить дополнительно», «нужна дополнительная проверка перед взаимодействием»;
...
Карточку и расчёты проверяй по данным инструментов, не считай их безусловно истинными.
Сопоставляй ИНН, показатель, знак, единицу, роль, статус и период. Правильная фраза для другой компании — ошибка.
Отсутствие поля нельзя заменять нулём; отсутствие деталей нельзя подменять отсутствием сводки.
Называть банкротство можно только при соответствующем основании, а не при любом закрытом статусе.
При сравнении проверь каждую компанию отдельно. Дата в метаданных без даты в видимом тексте не засчитывается.
Оцени только финальный видимый текст: отклонённые и удалённые утверждения не являются фактами ответа.
Не оценивай тон, вежливость и длину. Числа считай совпавшими при разумном округлении (26,2 млн ₽ = 26 249 000). Снижение применяй только если нарушение есть в тексте буквально; сомневаешься — не снижай. Комментарий пиши по-русски."""
...
def judge_messages(question: GoldQuestion, answer: Answer, tool_outputs: str) -> list:
    ...
    card_line = (
        "Карточка (проверь соответствие исходным данным инструментов): "
        f" {answer.card.model_dump(mode='json')}\n"
        if answer.card
        else ""
    )
    user = (
