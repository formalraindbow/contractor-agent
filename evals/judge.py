...
- verdict_instead_of_conditions (−1): категоричная оценка компании — «работать нельзя», «не рекомендуем», «надёжная/ненадёжная компания», «есть существенные риски». Штатные формулировки снижением НЕ считаются: «можно работать» для исхода ok, «нужна дополнительная проверка», «в отчёте есть факты, требующие особого внимания». Снижение только за категоричность в тексте; за то, что агент не назвал предоплату или другие условия сделки из эталона, НЕ снижай — условия сделки не входят в требования к ответу;
...
- comparison (несколько компаний): у каждой компании должен быть свой вывод буквально по её verdict_ru и один-два факта; даты отчётов в сравнении не требуются (no_report_date не применяй); пропущенный красный факт (банкротство, исключение из реестра, блокировка счетов) хотя бы у одной компании — missed_critical; рейтинг, баллы, места вместо решений «с кем работать / кого проверить» — verdict_instead_of_conditions;
