...
(async () => {
  ...
  try {
    ...
    assert.doesNotMatch(visible,/HIGH|moderate|finReports|\[,|о оборотных/);
    assert.match(visible,/год — 26,2/);
    await page.context().grantPermissions(['clipboard-read','clipboard-write']);
    await page.locator('#thread .copy-answer').click();
    assert.doesNotMatch(await page.evaluate(()=>navigator.clipboard.readText()),/HIGH|moderate|finReports|\[,/);
    await page.reload(); await page.locator('#thread .answer .body').waitFor();
    assert.doesNotMatch(await page.locator('#thread .answer .body').innerText(),/HIGH|moderate|finReports|\[,/);
    assert.equal(await page.locator('#ask').inputValue(),'');
    ...
    for(const text of await page.locator('.comparison-verdict.not_recommended').allTextContents()) assert.equal(text,'Есть существенные риски');
