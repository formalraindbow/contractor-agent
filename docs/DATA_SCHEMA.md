...
`arbitrationByStatus` и разбивка расходятся по присутствию у 12 из 200 (JSON 6 и 2
в разные стороны, CSV 4 и 0). Там, где есть оба (99), сводка больше суммы по годам
у 55 — это не ошибка: разбивка охватывает только 2023–2026, сводка — всё время.
Реальных расхождений 5 (сводка меньше разбивки) плюс 22, где `commonCount` больше
суммы шести вложенных счётчиков — дела без ролей. Флаг банка `arbitrationDefendant`
считается по разбивке (67 из 67), не по сводке.
...
- **Метки.** `baseInfo.riskLevel` — «Светофор», внутренний скоринг банка (LOW / MEDIUM / HIGH;
  UNKNOWN — серый, «нет данных для оценки»). `zskRiskLevel` — ЗСК, агрегация платформы ЦБ
  «Знай своего клиента» (GREEN / YELLOW / RED). В интерфейсе банка ЗСК выводится как
  Green / grey / grey — жёлтый и красный клиенту не показываются.
- **`status.status`** бывает `CURRENT` и `CLOSED` (ликвидированная); в снапшоте — только CURRENT.
  **`status.reasonName`** по спецификации — «причина закрытия организации»: то, что у 6
  действующих компаний это поле заполнено, значит, что процесс закрытия или изменения
  статуса идёт. `status.date` — дата последнего обновления статуса.
- **Поля, которых в снапшоте нет:** `baseInfo.staff` (диапазон численности персонала — тот
  самый вопрос «сколько человек в штате»), `phones[].phoneType`, `procurements[].tenderAdmittedCnt`,
  `foundersInfo.parentOrganizations[]` (в данных `parentOrganizations` встречается только внутри
  `relatedCompanies[]`), `foundersInfo.cofounders[].isActive` (в данных поле называется `active`).
  В модели они опциональны: в снапшоте отсутствуют, в реальном API могут прийти.
- **`arbitrationByStatus`:** Finished — закрытые дела, Pending — открытые, Appealed — обжалованные;
  «в процессе» = Pending + Appealed.
- **`inspections[].inspectionStatus`** — пять состояний: предстоящая / завершена без нарушений /
  завершена с нарушениями / результат неизвестен / отменена. **`licenses[].status`** —
  ACTIVE / EXPIRED / INDEFINITE.
- **`coefficient`:** названия совпадают с нашей численной расшифровкой — `sustainability`
  «коэффициент финансовой устойчивости» = (капитал + долгосрочные) / активы,
  `solvency` «платёжеспособности» = активы / (краткосрочные + долгосрочные),
  `profitability` «рентабельности» = прибыль / выручка × 100. Текущая ликвидность — не он.
